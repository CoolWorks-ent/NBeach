	...
	public class RichPath {
		...
		public void Initialize (Seeker seeker, Path path, bool mergePartEndpoints, bool simplificationMode) {
			...
			for (int i = 0; i < parts.Count; i++) {
				...
				if (funnelPart != null) Pathfinding.Util.ObjectPool<RichFunnel>.Release(ref funnelPart);
				else if (specialPart != null) Pathfinding.Util.ObjectPool<RichSpecial>.Release(ref specialPart);
			}
			...
			for (int i = 0; i < nodes.Count; i++) {
				if (nodes[i] is TriangleMeshNode) {
					...
					RichFunnel f = Pathfinding.Util.ObjectPool<RichFunnel>.Claim().Initialize(this, graph);
					...
					RichSpecial rps = Pathfinding.Util.ObjectPool<RichSpecial>.Claim().Initialize(nl, nodes[sIndex]);
					parts.Add(rps);
				}
			}
		}
